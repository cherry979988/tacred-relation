...
from data.loader import DataLoader, get_length
from model.ssvae import SSVAE
from utils import scorer, constant, helper
from utils.vocab import Vocab

from utils.mem_track import MemTracker
import inspect
...
gpu_tracker = MemTracker(frame, path='./logs/')
...
unlabeled_batch = DataLoader(opt['data_dir']  + '/train_unlabeled_%.2f.json' % opt['ratio'], opt['batch_size'], opt, vocab, evaluation=False)
...
# model
model = SSVAE(opt, emb_matrix=emb_matrix)
...
max_steps = len(labeled_batch) * opt['num_epoch']

# start training
for epoch in range(1, opt['num_epoch'] + 1):
    train_loss = 0

    for i, batch in enumerate(labeled_batch):
        start_time = time.time()
        global_step += 1
        loss = model.labeled_update(batch)
        train_loss += loss

        batch = unlabeled_batch[(global_step - 1) % len(unlabeled_batch)]
        model.unlabeled_update(batch)
    ...
    # todo 3.
    # todo 5. Generative Part rewrite...
    ...
    train_loss = train_loss / labeled_batch.num_examples * opt['batch_size']  # avg loss per batch
